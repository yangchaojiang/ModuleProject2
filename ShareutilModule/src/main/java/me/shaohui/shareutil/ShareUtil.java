...
public class ShareUtil {
    ...
    public static void shareText(Context context, @SharePlatform.Platform int platform, String text,
        ...
        if (!ShareUtil.isInstalled(platform, context)) return;
        mType = TYPE_TEXT;
        mText = text;
        mPlatform = platform;
    ...
    public static boolean isInstalled(@SharePlatform.Platform int platform, Context context) {
        switch (platform) {
            case SharePlatform.QQ:
            case SharePlatform.QZONE:
                boolean s = isQQInstalled(context);
                if (!s) YUtils.Toast("手机没有安装QQ");
                return s;
            case SharePlatform.WEIBO:
                s = isWeiBoInstalled(context);
                if (!s) YUtils.Toast("手机没有安装微博");
                return s;
            case SharePlatform.WX:
            case SharePlatform.WX_TIMELINE:
                s = isWeiXinInstalled(context);
                if (!s) YUtils.Toast("手机没有安装微信");
                return s;
            case SharePlatform.DEFAULT:
                return true;
            default:
